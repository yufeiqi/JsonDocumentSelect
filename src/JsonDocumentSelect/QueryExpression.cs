...
namespace JsonDocumentSelect
{
    ...
    internal abstract class QueryExpression
    {
        ...
        protected QueryExpression(QueryOperator @operator)
        {
            Operator = @operator;
        }
    ...
    internal class CompositeExpression : QueryExpression
    {
        ...
        public override bool IsMatch(JsonElement root, JsonElement t)
        {
            ...
            {
                if (Expressions.Any(e => !e.IsMatch(root, t)))
                {
                    return false;
                }
    ...
    internal class BooleanQueryExpression : QueryExpression
    {
        ...
        private static IEnumerable<JsonElementExt> GetResult(JsonElement root, JsonElement t, object o)
        {
            if (o is JsonElement resultToken)
            {
                return new [] { new JsonElementExt(){ Element=resultToken } };
            }

            if (o is List<PathFilter> pathFilters)
            {
                return JsonDocumentSelect.Evaluate(pathFilters, root, t, false);
            }
            ...
            using (IEnumerator<JsonElementExt> leftResults = GetResult(root, t, Left).GetEnumerator())
            {
                if (leftResults.MoveNext())
                {
                    ...
                    do
                    {
                        ...
                        if (rightResults.Any(rightResult =>
                            rightResult.Element != null && leftResult != null &&
                            MatchTokens(leftResult.Value, rightResult.Element.Value)))
                        {
                            return true;
                        }
        ...
        private bool MatchTokens(JsonElement leftResult, JsonElement rightResult)
        {
            ...
            {
                if (OperatorMatch(leftResult, rightResult))
                {
                    return true;
                }
            }
            else if (Operator == QueryOperator.Exists || Operator == QueryOperator.NotEquals)
            {
                 return true;
            }
        ...
        private bool OperatorMatch(JsonElement leftResult, JsonElement rightResult)
        {
            switch (Operator)
            {
        ...
        internal static bool EqualsWithStringCoercion(JsonElement value, JsonElement queryValue)
        {
            ...
            {
                return Math.Abs(value.GetDouble() - queryValue.GetDouble()) < 0.01;
            }
